...
public class OrderSpecifierUtil {
    // 동적 정렬 정보 생성 메서드
    public static <T> OrderSpecifier<?>[] getOrderSpecifier(Pageable pageable, EntityPathBase<T> qClass) {
        ...
        // `OrderSpecifierUtil.getOrderSpecifier()`의 경우 정렬 기준이 복수로도 오는걸 전제로 하여,
        ...
        //      ]
        // 그러면 `OrderSpecifierUtil.getOrderSpecifier()`를 호출한 곳에서는 아래와 같은 형태가 됨
        ...
        //      - `OrderSpecifierUtil.getOrderSpecifier()`의 리턴 타입이 OrderSpecifier<?>[]로 지정되기 때문에 컴파일 중 타입 에러 발생 방지하여 타입 안정성 제공
